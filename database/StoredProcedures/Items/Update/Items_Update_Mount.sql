DROP PROCEDURE IF EXISTS Items_Update_Mount;

DELIMITER ;;
CREATE PROCEDURE Items_Update_Mount(
...
BEGIN
    ...
    IF valid THEN
        UPDATE items
        SET name = itemName, description = itemDescription,
            cost = costValue, cost_unit = costUnit, weight = weightValue, version = version + 1
        WHERE (user_id = userId Or userId = 19) AND id = itemId;         # (user_id = userId Or userId = 19), edited by Mark
        
        UPDATE mounts
        SET speed = speedValue, carrying_capacity = carryingCapacity
...
# CALL Items_Update_Mount(265, 1, 'test-changed', 'test-changed-desc', 'ab');
